 ...
 * Tiap alasan berupa objek { aspek, nilai, target }, bukan kalimat jadi.
 * Bentuk terurai ini yang memungkinkan tampilan menyusunnya sebagai
 * lencana ringkas -- "B. Indo 84 / 90" -- alih-alih satu kalimat panjang
 * yang harus dibaca dari ujung ke ujung untuk tahu ada berapa aspek yang
 * bermasalah. Jumlah aspek itu sendiri yang dipakai mengurutkan daftar.
 *
    ...
    .map((b) => {
      ...
      MAPEL.forEach((m) => {
        if (b[m.kunci] !== null && b[m.kunci] !== undefined && b[m.kunci] < target) {
          alasan.push({ aspek: m.pendek, nilai: bulat(b[m.kunci]), target });
        }
      });
