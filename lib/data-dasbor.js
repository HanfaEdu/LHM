 ...
 *
 * Beda dari muatLogWa(): nis di akses_dibuka BENAR-BENAR kunci asing ke
 * siswa, jadi bisa disisipkan (embed) langsung lewat satu query
 * PostgREST -- tidak perlu digabung manual di sini.
 */
export async function muatLogAksesRapor() {
  const [
    { data: riwayat, error: e1 },
    { data: siswa, error: e2 },
    { data: penempatan, error: e3 },
    { data: akses, error: e4 },
  ] = await Promise.all([
    supabase
      .from('akses_dibuka')
      ...
      .limit(2000),
    supabase.from('siswa').select('nis, nama_lengkap, nama_panggilan'),
    supabase
      .from('penempatan')
      .select('nis, kelas:kelas_id (nama_kelas, tahun_ajaran)'),
    supabase.from('akses_ortu').select('nis, aktif, terakhir_dibuka'),
  ]);
  ...
  // Kelas TERKINI seorang siswa: tahun ajaran tertinggi menang -- sama
  // seperti pola di app/api/wa/route.js, supaya anak yang sudah naik
  // kelas tidak disebut kelas lamanya.
  ...
  // Satu nis bisa saja punya lebih dari satu baris akses_ortu di masa
  // lalu (token diganti membuat baris baru pada sebagian instalasi lama);
  // yang dipakai untuk ringkasan cukup salah satu yang aktif, atau yang
  // paling baru kalau tidak ada yang aktif.
  const aksesPer = new Map();
  for (const a of akses || []) {
    const sebelumnya = aksesPer.get(a.nis);
    if (!sebelumnya || (a.aktif && !sebelumnya.aktif)) aksesPer.set(a.nis, a);
  }
  ...
  const ringkasan = (siswa || []).map((s) => {
    const a = aksesPer.get(s.nis);
    const jumlahDibuka = jumlahPer.get(s.nis) || 0;
    // akses_ortu.terakhir_dibuka dipakai sebagai cadangan: rapor yang
    // dibuka SEBELUM migrasi 006 tidak akan pernah muncul di akses_dibuka,
    // dan tanpa ini anak itu tampak seolah belum pernah dibuka sama sekali.
    const terakhirDibuka = terakhirLogPer.get(s.nis) || a?.terakhir_dibuka || null;

    let status;
    if (!a || !a.aktif) status = 'belum_terbit';
